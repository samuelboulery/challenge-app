-- Reset group operational data while keeping the group itself.
-- Owner only: remove group-related points impact, purchased items, challenges, and proof images.
...
BEGIN
  ...
  -- Purge proof images from storage bucket for this group's challenges.
  ...
  WHERE so.bucket_id = 'proofs'
    AND so.name = proof_paths.path;
