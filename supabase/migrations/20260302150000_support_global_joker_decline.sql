-- ============================================================
-- Support joker global in decline_with_penalty
-- ============================================================

CREATE OR REPLACE FUNCTION public.decline_with_penalty(
  p_challenge_id UUID,
...
DECLARE
  ...
  v_joker_global_item global_shop_items%ROWTYPE;
  v_result jsonb;
BEGIN
  ...
  IF v_weekly_declines >= 2 THEN
    IF p_joker_inventory_id IS NOT NULL THEN
      ...
      IF v_joker_item.shop_item_id IS NOT NULL THEN
        ...
        END IF;
      ELSE
        SELECT * INTO v_joker_global_item
          FROM global_shop_items
          WHERE id = v_joker_item.global_shop_item_id
            AND item_type = 'joker'
            AND is_active_global = true;

        IF NOT FOUND OR v_joker_item.purchased_group_id IS DISTINCT FROM v_challenge.group_id THEN
          RAISE EXCEPTION 'Invalid joker for this group';
        END IF;
      END IF;
