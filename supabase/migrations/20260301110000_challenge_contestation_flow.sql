-- ============================================================
-- Challenge contestation flow by target + group vote
-- ============================================================
...
ALTER TABLE challenge_price_votes
  ...
  CHECK (vote IN ('approve', 'reject', 'counter', 'cancel'));

-- ============================================================
-- RPC: start_challenge_contestation
-- Target opens a group vote phase (status -> negotiating)
-- ============================================================
...
-- ============================================================
-- RPC: vote_challenge_contestation
-- Eligible members vote cancel OR counter(amount)
-- First option to threshold wins.
-- ============================================================
...
BEGIN
  ...
  IF p_vote NOT IN ('counter', 'cancel') THEN
  ...
  IF p_vote = 'cancel' AND p_counter_points IS NOT NULL THEN
    RAISE EXCEPTION 'Invalid counter proposal';
  END IF;
  ...
  IF v_cancel_votes >= v_threshold THEN
    UPDATE challenges
  ...
  IF v_counter_votes >= v_threshold THEN
...
-- ============================================================
-- Refresh state RPC for contestation progress UI
-- approvals=counter votes, rejections=cancel votes
-- ============================================================
...
REVOKE ALL ON FUNCTION start_challenge_contestation(UUID) FROM PUBLIC;
REVOKE ALL ON FUNCTION vote_challenge_contestation(UUID, TEXT, INT) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION start_challenge_contestation(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION vote_challenge_contestation(UUID, TEXT, INT) TO authenticated;
